import"./modulepreload-polyfill-Btlm8H0F.js";import{_ as e,a as t,d as n,f as r,i,n as a,p as o,t as s,x as c}from"./firebase-VBRKn9At.js";import{n as l,t as u}from"./toast-kV3jSCF7.js";/* empty css               */import{a as d,d as f,f as p,i as m,l as h,n as g,p as _,r as v,u as y}from"./tournament-rules-D9Tq3W95.js";var b=new URLSearchParams(window.location.search).get(`id`),x=null,S=null,C=[],w=null,T=null;function E(){return Array.from(document.querySelectorAll(`input[name="registrationCategory"]:checked`)).map(e=>e.value.trim()).filter(Boolean)}function D(e){return e?new Intl.DateTimeFormat(`pt-BR`,{dateStyle:`medium`}).format(e):`Nao informado`}function O(e,t){return e?!t||t===e?D(e):`${D(e)} ate ${D(t)}`:`Nao informado`}function k(e){return e===void 0?`Nao informado`:new Intl.NumberFormat(`pt-BR`,{style:`currency`,currency:`BRL`}).format(e)}function A(e){return p(e)||e.doubleRegistrationFee===void 0?k(e.registrationFee):`${k(e.registrationFee)} (1 cat.) / ${k(e.doubleRegistrationFee)} (2 cats.)`}function j(e){return e.status===`finished`||e.status===`closed`?!0:e.status===`open`?!1:!!(e.registrationDeadline&&e.registrationDeadline<Date.now())}function M(){let e=document.getElementById(`registrationModal`),t=document.getElementById(`registrationModalText`),n=document.getElementById(`registrationCategoryOptions`),r=S;if(!e||!t||!n||!r)return;let i=v(r,x?.category);if(!i.length){l(`Sua categoria atual nao permite inscrição neste torneio.`,`warning`);return}t.textContent=p(r)?`Escolha a categoria do ranking para se inscrever em ${r.title}.`:`Escolha uma ou duas categorias para se inscrever em ${r.title}.`,n.innerHTML=i.map((e,t)=>{let n=f(r,C,e),i=d(C,e),a=m(r,e);return`
  ...
  `,r.innerHTML=`<p>${s.description||`Texto do torneio ainda nao definido. Depois voce pode editar essa apresentação no cadastro do torneio.`}</p>`,p(s)?(i.style.display=`block`,a.innerHTML=C.length?[...C].filter(e=>e.paymentStatus===`approved`).sort((e,t)=>e.name.localeCompare(t.name)).map(e=>`
    ...
    <div class="stack-item">
      <div class="stack-item-header">
        <div>
          <strong>${l}</strong>
          <span>${p(s)?`Ranking com visao completa dos inscritos e das informacoes do evento.`:`Campeonato com informacoes gerais antes de seguir para a inscrição.`}</span>
        </div>
      </div>
      <div class="stack-item-grid">
        ...
        <span>Status da inscrição: ${w===`pending_payment`?`pendente de aprovação`:w===`approved`?`aprovada`:`nao enviada`}</span>
      </div>
