import"./modulepreload-polyfill-Btlm8H0F.js";import{_ as e,a as t,f as n,i as r,n as i,o as a,p as o,s,t as c,x as l}from"./firebase-VBRKn9At.js";/* empty css               */import{c as u,f as d,s as f}from"./tournament-rules-CmdOaKKb.js";import{n as p}from"./ranking-standings-Dec7PvHv.js";var m=null,h=[],g=[],_=new Map,v=new Map,y=null,b=null,x=null,S=null,C=``,w=[`A`,`B`,`C`,`D`,`Iniciante`];function T(e){return(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}function E(e){return e?new Intl.DateTimeFormat(`pt-BR`,{dateStyle:`medium`}).format(e):`Não informado`}function D(e){return e?new Intl.DateTimeFormat(`pt-BR`,{dateStyle:`medium`,timeStyle:`short`}).format(e):`Não informado`}function O(e){let t=(e??``).trim().toUpperCase();return t===`A`?`A`:t===`B`?`B`:t===`C`?`C`:t===`D`?`D`:t===`INICIANTE`||t===`INICIANTES`?`Iniciante`:null}function k(e){return g.find(t=>t.id===e)??null}function A(e){return h.find(t=>(t.tournamentId||t.id)===e)??null}function j(e){return Array.isArray(e.categories)&&e.categories.length?e.categories:e.category?e.category.split(`,`).map(e=>e.trim()).filter(Boolean):[]}function M(e,t){let n=(Array.isArray(e.categories)?e.categories:[]).map(e=>O(e)).filter(Boolean),r=j(t).map(e=>O(e)).filter(Boolean),i=n.length?r.filter(e=>n.includes(e)):r;return i.length?[...new Set(i)]:[...new Set(n)]}function N(e){let t=e.category?.trim();if(t)return t;if(Array.isArray(e.categories)){let t=e.categories.map(e=>e.trim()).find(Boolean);if(t)return t}return`A`}function P(e){let t=new Set;return(e?.queue??[]).forEach(e=>{e.playerIds?.forEach(e=>{e&&t.add(e)})}),(e?.activeTables??[]).forEach(e=>{e.playerIds?.forEach(e=>{e&&t.add(e)})}),t}function F(e,t){let n=M(e,t),r=Array.isArray(e.categories)&&e.categories.length?e.categories:n;return n.filter(e=>r.includes(e))}function I(e){let t=Object.entries(e.championshipState??{}).filter(([,e])=>(e?.groups?.length??0)>0).map(([e])=>O(e)).filter(Boolean),n=(Array.isArray(e.categories)?e.categories:[]).map(e=>O(e)).filter(Boolean);return[...new Set([...t,...n])].sort((e,t)=>w.indexOf(e)-w.indexOf(t))}function L(e,t){let n=t.map(e=>O(e)).filter(Boolean);if(!n.length)return{label:e.status===`finished`?`Finalizado`:`Em andamento`,tone:e.status===`finished`?`neutral`:`win`};let r=n.map(t=>e.championshipState?.[t]).filter(e=>e?.finished).length;return r===n.length?{label:`Finalizado`,tone:`neutral`}:r>0?{label:`Parcial`,tone:`neutral`}:{label:e.status===`closed`?`Inscricoes encerradas`:e.status===`open`?`Em andamento`:`Aguardando`,tone:`win`}}function R(e,t){if(d(e))return e.status!==`finished`;let n=I(e);return n.length?n.some(t=>!e.championshipState?.[t]?.finished):e.status!==`finished`}function z(e){return new Map(B(e).map(e=>[e.id,e.name]))}function B(e){return(_.get(e)??[]).filter(e=>e.paymentStatus===`approved`)}function V(e,t=13){let n=e.trim();if(n.length<=t)return n;let r=n.split(/\s+/).filter(Boolean);if(r.length>=2){let e=r[0],n=r[r.length-1],i=r.slice(1,-1).filter(e=>![`de`,`da`,`do`,`dos`,`das`,`e`].includes(e.toLowerCase()));if(i.length){let r=`${e} ${i[0][0]}. ${n}`;if(r.length<=t+6)return r;let a=`${e} ${i.map(e=>`${e[0]}.`).join(` `)} ${n}`.replace(/\s+/g,` `).trim();if(a.length<=t+8)return a}let a=`${e} ${n}`;if(a.length<=t+4)return a}return`${n.slice(0,Math.max(1,t-3)).trimEnd()}...`}function H(){let e=document.getElementById(`tournamentViewerModal`);if(!e)return;let t=Array.from(e.querySelectorAll(`.ranking-completed-card`)),n=e.querySelector(`.ranking-filter-empty`),r=e.querySelector(`.ranking-history-count`),i=0;t.forEach(e=>{let t=(e.dataset.search||``).toLowerCase(),n=!C||t.includes(C.toLowerCase());e.style.display=n?``:`none`,n&&(i+=1)}),r&&(r.textContent=i?`${i} confronto(s)`:`Sem jogos exibidos`),n&&(n.style.display=i?`none`:`block`)}function U(e,t){let n=new Map;return(_.get(e)??[]).filter(e=>e.paymentStatus===`approved`).forEach(e=>{n.set(e.id,{wins:0,losses:0,games:0})}),(v.get(e)??[]).filter(e=>(e.group??`general`)===t).forEach(e=>{let t=n.get(e.p1),r=n.get(e.p2);t&&(t.games+=1),r&&(r.games+=1),e.winner===e.p1?(t&&(t.wins+=1),r&&(r.losses+=1)):(r&&(r.wins+=1),t&&(t.losses+=1))}),n}function W(e,t){let n=[];for(let r=0;r<t.playerIds.length;r++)for(let i=r+1;i<t.playerIds.length;i++)n.push({id:`${e}_${t.id}_${t.playerIds[r]}_${t.playerIds[i]}`,stage:`groups`,category:e,groupId:t.id,playerIds:[t.playerIds[r],t.playerIds[i]]});return n}function G(e,t,n){let r=new Map;return e.playerIds.forEach(e=>{r.set(e,{wins:0,losses:0,scored:0,conceded:0})}),(t.completedMatches??[]).filter(t=>t.stage===`groups`&&t.groupId===e.id).forEach(e=>{let[t,n]=e.playerIds,i=r.get(t),a=r.get(n);if(!i||!a)return;let o=e.score1??0,s=e.score2??0;i.scored+=o,i.conceded+=s,a.scored+=s,a.conceded+=o,e.winnerId===t?(i.wins+=1,a.losses+=1):e.winnerId===n&&(a.wins+=1,i.losses+=1)}),[...r.entries()].map(([e,t])=>({playerId:e,...t})).sort((e,t)=>{if(t.wins!==e.wins)return t.wins-e.wins;let r=e.scored-e.conceded,i=t.scored-t.conceded;return i===r?(n.get(e.playerId)||`Atleta`).localeCompare(n.get(t.playerId)||`Atleta`):i-r})}function K(e,t,n,r,i=``){return`
  ...
  `.replace(/Â°/g,`o`),H(),i.style.display=`flex`}function J(e,t){let n=document.getElementById(`tournamentViewerTitle`),r=document.getElementById(`tournamentViewerContent`),i=document.getElementById(`tournamentViewerModal`),a=m;if(!n||!r||!i||!a)return;let o=I(e),s=o.includes(t)?t:o[0]??t,c=e.championshipState?.[s],l=z(e.id),u=c?.groups??[],d=u.length>0,f=!!c?.finished,p=u.some(e=>e.id===S)?S:u[0]?.id,h=(c?.completedMatches??[]).filter(e=>e.stage===`groups`&&e.groupId===p).sort((e,t)=>(t.playedAt??0)-(e.playedAt??0)),g=(c?.activeTables??[]).filter(e=>e.match).map(e=>({left:l.get(e.match.playerIds[0])||`Atleta`,right:l.get(e.match.playerIds[1])||`Atleta`,meta:`Mesa ${e.id}`,detail:e.match?.roundTitle||e.match?.groupId||`Ao vivo`})),_=(c?.queue??[]).slice(0,10).map(e=>({left:l.get(e.playerIds[0])||`Atleta`,right:l.get(e.playerIds[1])||`Atleta`,meta:e.roundTitle||e.groupId||`Fila`,detail:e.stage===`knockout`?`Mata-mata`:`Grupos`}));if(n.textContent=`${e.title} - Categoria ${s}`,!d){r.innerHTML=`
    ...
    <div class="tournament-viewer-shell">
      ...
      ${f?``:K(`Proximos jogos`,`Fila`,_,`Nenhum proximo jogo liberado ainda.`,`tournament-feed-card championship-live-card`)}
  ...
  `,i.style.display=`flex`}function Y(){let e=document.getElementById(`myChampionshipsList`);if(!e)return;let t=h.filter(e=>{let t=k(e.tournamentId||e.id);return t?R(t,e):!1}).map(e=>{let t=k(e.tournamentId||e.id);if(!t)return``;let n=F(t,e),r=d(t)?`Ranking`:`Campeonato`,i=d(t)?{label:t.status===`finished`?`Finalizado`:t.status===`open`?`Em andamento`:t.status===`closed`?`Inscricoes encerradas`:`Aguardando`,tone:t.status===`finished`?`neutral`:`win`}:L(t,n);return`
