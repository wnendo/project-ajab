import"./modulepreload-polyfill-Btlm8H0F.js";import{_ as e,a as t,d as n,f as r,i,n as a,p as o,t as s,x as c}from"./firebase-VBRKn9At.js";import{n as l,t as u}from"./toast-kV3jSCF7.js";/* empty css               */import{a as d,d as f,f as p,i as m,l as h,n as g,p as _,r as v,u as y}from"./tournament-rules-D9Tq3W95.js";var b=new URLSearchParams(window.location.search).get(`id`),x=null,S=null,C=[],w=null,T=null,E=``,D=new Map;function O(){return Array.from(document.querySelectorAll(`input[name="registrationCategory"]:checked`)).map(e=>e.value.trim()).filter(Boolean)}function k(e,t){return v(e,t).filter(e=>e!==`Iniciante`)}function A(e){return e?new Intl.DateTimeFormat(`pt-BR`,{dateStyle:`medium`}).format(e):`Não informado`}function j(e,t){return e?!t||t===e?A(e):`${A(e)} até ${A(t)}`:`Não informado`}function M(e){return e===void 0?`Não informado`:new Intl.NumberFormat(`pt-BR`,{style:`currency`,currency:`BRL`}).format(e)}function N(e){return(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}function P(e){return e.split(` `).filter(Boolean).slice(0,2).map(e=>e[0]?.toUpperCase()??``).join(``)}function F(){return[...C].filter(e=>e.paymentStatus===`approved`).sort((e,t)=>e.name.localeCompare(t.name))}function I(){let e=F();return E?e.filter(e=>[e.name,e.club,e.category,...e.categories??[]].filter(Boolean).some(e=>e.toLowerCase().includes(E))):e}function L(e){return Array.from(new Set(e.map(e=>e.group))).map(t=>({group:t,entries:e.filter(e=>e.group===t)}))}function R(e){return`
    <div class="ranking-showcase">
      <div class="ranking-showcase-podium">
        ${e.slice(0,3).map((e,t)=>`
              <article class="ranking-showcase-podium-card place-${t+1}">
                <span class="ranking-showcase-place">${N(e.placement)}</span>
                <strong>${N(e.name)}</strong>
                <small>${N(e.category)}</small>
      ...
      <div class="ranking-showcase-table">
        ${e.map(e=>`
              <div class="ranking-showcase-row">
                <span>${N(e.placement)}</span>
                <strong>${N(e.name)}</strong>
                <small>${N(e.result)}</small>
                <span>${e.wins}V / ${e.losses}D</span>
              </div>
            `).join(``)}
      </div>
    </div>
  `}function z(e){return p(e)||e.doubleRegistrationFee===void 0?M(e.registrationFee):`${M(e.registrationFee)} (1 cat.) / ${M(e.doubleRegistrationFee)} (2 cats.)`}function B(e){return e.status===`finished`||e.status===`closed`?!0:e.status===`open`?!1:!!(e.registrationDeadline&&e.registrationDeadline<Date.now())}function V(){let e=document.getElementById(`registrationModal`),t=document.getElementById(`registrationModalText`),n=document.getElementById(`registrationCategoryOptions`),r=S;if(!e||!t||!n||!r)return;let i=k(r,x?.category);if(!i.length){l(`Sua categoria atual nao possui inscricao publica disponivel neste campeonato.`,`warning`);return}t.textContent=p(r)?`Escolha a categoria do ranking para se inscrever em ${r.title}.`:`Escolha uma ou duas categorias para se inscrever em ${r.title}.`,n.innerHTML=i.map((e,t)=>{let n=f(r,C,e),i=d(C,e),a=m(r,e);return`
      ...
      `}).join(``),e.style.display=`flex`}function H(){let e=document.getElementById(`rankingRegistrationsList`),t=document.getElementById(`rankingRegistrationsCount`);if(!e||!t)return;let n=I(),r=F().length;t.textContent=`${n.length} de ${r} atletas`,e.innerHTML=n.length?n.map(e=>`
            <div class="stack-item compact-stack-item ranking-registration-item">
              <div class="stack-item-header">
                <div>
                  <strong>${N(e.name)}</strong>
                  <span>${N(e.club||`Sem clube`)}</span>
                </div>
                <span class="result-pill neutral">${N(e.category||`Categoria`)}</span>
              </div>
              <div class="admin-tournament-actions">
                <button class="btn secondary" onclick="openRegisteredAthleteProfile('${e.id}')">Ver perfil</button>
              </div>
            </div>
          `).join(``):`<div class="empty-state">Nenhum atleta encontrado nesta busca.</div>`}function U(){let e=document.getElementById(`tournamentResultsCard`),t=document.getElementById(`tournamentResultsList`);if(!e||!t||!S?.finalStandings?.length){e&&(e.style.display=`none`);return}e.style.display=`block`,t.innerHTML=L(S.finalStandings).map(({group:e,entries:t})=>`
      ...
      `).join(``)}function W(){let e=document.getElementById(`tournamentResultsModalList`);!e||!S?.finalStandings?.length||(e.innerHTML=L(S.finalStandings).map(({group:e,entries:t})=>`
      ...
      `).join(``))}function G(e){let t=document.getElementById(`registeredAthleteProfileModal`),n=document.getElementById(`registeredAthleteProfileContent`),r=D.get(e),i=C.find(t=>t.id===e||t.uid===e);if(!t||!n||!r){l(`Nao foi possivel carregar o perfil deste atleta.`,`warning`);return}n.innerHTML=`
    <div class="athlete-profile-card">
      <div class="athlete-profile-head">
        <div class="athlete-profile-avatar-wrap">${r.photoURL?`<img class="profile-avatar" src="${N(r.photoURL)}" alt="Foto de ${N(r.name)}">`:`<div class="profile-avatar profile-avatar-fallback">${N(P(r.name))}</div>`}</div>
        <div class="athlete-profile-copy">
          <h3>${N(r.name)}</h3>
          <p>${N(r.club||`Sem clube`)} - ${N(r.category||`Sem categoria`)}</p>
          <div class="athlete-profile-badges">
            <span class="result-pill neutral">${N(i?.category||`Categoria`)}</span>
            <span class="result-pill win">Inscricao confirmada</span>
          </div>
        </div>
      </div>
      <div class="profile-info-grid athlete-profile-grid">
        <div class="info-card"><span>Email</span><strong>${N(r.email||`Nao informado`)}</strong></div>
        <div class="info-card"><span>Telefone</span><strong>${N(r.phone||`Nao informado`)}</strong></div>
        <div class="info-card"><span>Clube</span><strong>${N(r.club||`Nao informado`)}</strong></div>
        <div class="info-card"><span>Categoria base</span><strong>${N(r.category||`Nao informada`)}</strong></div>
        <div class="info-card"><span>Categoria no torneio</span><strong>${N(i?.category||`Nao informada`)}</strong></div>
        <div class="info-card"><span>Cadastro</span><strong>${A(r.createdAt)}</strong></div>
      </div>
    </div>
  `,t.style.display=`flex`}function K(){let e=document.getElementById(`tournamentDetailsTitle`),t=document.getElementById(`tournamentDetailsSubtitle`),n=document.getElementById(`tournamentDetailsInfo`),r=document.getElementById(`tournamentDetailsDescription`),i=document.getElementById(`rankingRegistrationsCard`),a=document.getElementById(`tournamentDetailsRegistration`),o=S;if(!e||!t||!n||!r||!i||!a||!o)return;e.textContent=o.title,t.textContent=`${o.location||`Local a definir`} - confira as informações antes de seguir para a inscrição.`,n.innerHTML=`
    <div class="info-card"><span>Tipo</span><strong>${y(o)===`ranking`?`Ranking`:`Campeonato`}</strong></div>
    <div class="info-card"><span>Data</span><strong>${j(o.startDate,o.endDate)}</strong></div>
    <div class="info-card"><span>Local</span><strong>${o.location||`Local a definir`}</strong></div>
    <div class="info-card"><span>Status</span><strong>${o.status===`finished`?`Finalizado`:B(o)?`Inscrições encerradas`:`Inscrições abertas`}</strong></div>
  `,r.innerHTML=`<p>${o.description||`Texto do torneio ainda não definido. Depois você pode editar essa apresentação no cadastro do torneio.`}</p>`,p(o)?(i.style.display=`block`,H(),U(),W()):i.style.display=`none`;let s=k(o,x?.category).some(e=>!f(o,C,e)),c=w===`approved`?`Inscrito`:w===`pending_payment`?T===`pay_on_day`?`Pagar no dia - pendente`:`Pagamento em análise`:s?`Inscreva-se`:`Categoria lotada`,l=!!(o.pixKey&&o.pixHolder);a.innerHTML=`
    <div class="stack-item tournament-registration-panel">
      ...
      <div class="stack-item-grid">
        <span>Valor: ${z(o)}</span>
        <span>Inscrições até: ${A(o.registrationDeadline)}</span>
